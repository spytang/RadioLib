...
int16_t LoRaWANNode::sendReceive(const uint8_t* dataUp, size_t lenUp, uint8_t fPort, uint8_t* dataDown, size_t* lenDown, bool isConfirmed, LoRaWANEvent_t* eventUp, LoRaWANEvent_t* eventDown) {
  ...
  for(; trans < this->nbTrans; trans++) {
    ...
    // [修改] 开始：根据 isFirstDataUplink 标志决定是否打开接收窗口
    if (this->isFirstDataUplink) {
      // 这是首次数据上行，行为正常
      RADIOLIB_DEBUG_PROTOCOL_PRINTLN("First data uplink after join, opening RX windows.");
      ...
      // 非首次上行，跳过接收窗口以加快发送频率
      RADIOLIB_DEBUG_PROTOCOL_PRINTLN("Skipping RX windows for faster transmission.");
      // 模拟“未收到下行”的情况，返回值为 0
      state = 0; 
    }
    // [修改] 结束
...
// [新增] 移出循环外设置：只要成功完成了一次发送循环（无论是否接收），就将首次标志位置为 false
this->isFirstDataUplink = false;
...
void LoRaWANNode::adrBackoff() {
  ...
  if(this->band->bandType == RADIOLIB_LORAWAN_BAND_DYNAMIC) {
    // in a dynamic band, default channels are never modified, so safe to assume they exist
    for(int num = 0; num < 3; num++) {
      if(this->band->txFreqs[num].enabled) {
        this->channelPlan[RADIOLIB_LORAWAN_UPLINK][num].enabled = true;
      }
    }
  } else {
